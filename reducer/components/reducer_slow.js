...
const try_remove = async (racy_item, safe_item, is_multiline, safeShader, racyShader, mismatches, shaderInfo, parser, mismatchMode, specificMismatches, init) => {
    ...
    if (safe_item) {
        ...
        if (can_remove) {
            ...
            new_safeShader = await remove(safe_item.node, safeShader, parser);
            new_racyShader = await remove(racy_item.node, racyShader, parser);
...
const getMismatches = async (safeShader, racyShader, shaderInfo, retries = 3, delay = 1000) => {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            const outputs1 = await run_shader(safeShader, shaderInfo, init);
            console.log("ran shader 1 successfully");
            init = await run_init(shaderInfo);
            const outputs2 = await run_shader(racyShader, shaderInfo, init);
...
const remove = async (node, code, parser, retries = 3, delay = 1000) => {
    ...
    //console.log("reduced code ", reduced_code);
